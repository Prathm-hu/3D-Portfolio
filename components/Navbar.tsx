...
export const Navbar = () => {
  ...
  return (
    <>
      <div className="z-50 fixed flex justify-center w-full test-white font-bold">
        ...
        <div
          className={`fixed left-0 top-0 w-full h-full bg-black/90 transform transition-transfrom duration-300
        ${nav ? 'translate-x-0' : '-translate-x-full'}`}>
          <ul className="flex flex-col items-center justify-center space-y-8 h-full">
          {navLinks.map((links, index) => (
              <li key={index}>
                <Link
                  href={links.path} onClick={toggleNav}
                  className="transform hover:text-white/50 transition-all duration-300 ease-in-out"
                >
                  {links.title}
                </Link>
